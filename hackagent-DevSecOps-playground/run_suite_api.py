...
def main():
    ...
    if start_from and start_from in all_strategies:
        ...
        logger.info(f"Ripresa dei test a partire da: '{start_from}' ({len(strategies_to_run)} test rimanenti)")
    else:
    ...
    logger.info(f"Avvio Test Suite Completa (run_suite): {len(test_configs)} test pianificati con Attaccante: {ATTACKER_MODEL}")
    ...
    for i, config in enumerate(test_configs):
        ...
        logger.info(f"ID: {run_id} | Strategia: {strategy} | Target: {target_model} | Attacker: {attacker_model} | Hardening: {AGENT_HARDENING}")
